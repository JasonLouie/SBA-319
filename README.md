...
| 204 - No Content | Successfully deleted entry. Currently there isn't any cascade delete for reviews when an anime or user is removed. |
...
The users model contains a name, username, email, and password. These are the chosen fields because most review sites allow names to be repeated (display names), but usernames must be unique. Emails also must be unique, but can be changed to another one (that does not already exist in the db). The anime models should include a description/summary field, but I kept the basics so that creating the seed data would be easier (descriptions/summaries for anime can be lengthy). A review has an optional comment field because users should be able to rate content without having to leave a comment. Ratings are integers between 0 - 10 because of personal preference.

## Testing

Creating reviews using the demo might be difficult because user ids are 24 characters long, so copy and pasting is necessary. Most of the functionality can be tested using the demo endpoint.

## Endpoints (API)

The base url for the API endpoint is `localhost:3000/api`.
...
| GET | `/users/seed` | Resets users collection and adds static data | Code is 200 if old documents were successfully removed and new ones were successfully added. Returns the new documents added.<br>Code is 400 if any of the new documents fail validation or an error occurred when deleting old documents. |
...
| GET | `/anime/seed` | Resets anime collection and adds static data | Code is 200 if old documents were successfully removed and new ones were successfully added. Returns the new documents added.<br>Code is 400 if any of the new documents fail validation or an error occurred when deleting old documents. |
...
| GET | `/reviews/seed` | Resets reviews collection and adds static data | Code is 200 if old documents were successfully removed and new ones were successfully added. Returns the new documents added.<br>Code is 400 if any of the new documents fail validation or an error occurred when deleting old documents. |
...
The base url for the demo endpoint is `localhost:3000/demo`, but each endpoint below will include the `/demo` prefix. The same validation logic from the previous section applies to the respective document type. Errors are rendered. The title shows the error type and the content in the main body of the page lists out all errors with key: value (key being the field or model with the error and the value being the error).

| Method | Endpoint | Description & Response |
| --- | ------ | ------ | ----- |
| GET | `/demo/users`<br>`/demo/anime`<br>`/demo/reviews` | Retrieves all documents of that type and renders the respective view `{type}/index.ejs`. |
| GET | `/demo/users/:id`<br>`/demo/anime/:id`<br>`/demo/reviews/:id` | Retrieves a particular document with that id and renders the respective view `{type}/doc.ejs`. Any errors are shown using `error.ejs`. |
| POST | `/demo/users`<br>`/demo/anime`<br>`/demo/reviews` | Creates a new doc (user, anime, or review). | It must pass the validation specified in the Models and Schema table. Returns the new document created and renders it in the respective view `{type}/doc.ejs` if the creation was successful. Any errors are shown using `error.ejs`. |
| PATCH | `/demo/users/:id`<br>`/demo/anime/:id`<br>`/demo/reviews/:id` | Finds and updates a particular document with that id and renders the respective view `{type}/doc.ejs` if the update was successful. Any errors are shown using `error.ejs`. |
| DELETE | `/demo/users/:id`<br>`/demo/anime/:id`<br>`/demo/reviews/:id` | Finds and deletes a particular document with that id and renders the respective view `{type}/doc.ejs` if the deletion was successful. Any errors are shown using `error.ejs`. |